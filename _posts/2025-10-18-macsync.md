...
On **October 17th, 2025** I came across a malicious file `update` that appeared to be related to the MacSync InfoStealer. Through some initial research I was able to stumble across what was very likely the initial `curl` command served to users by a malicious `Paste & Run` webpage:
...
- Rip apart the related applescript pulled down by the `update` file. 
...
This is the specific instance of `curl` where the malicious applescript is downloaded and piped to `osascript`. If you are executing this in a sandbox, you will notice the applescript pulled down from the `goalbus` url is stored in the `STD_OUT`. This is because the output isn't written to disk, it is stored in memory before being piped to `osascript`. 
...
If we wanted to execute this script for some dynamic analysis efforts, we could simply remove this line from the script. If this were a compiled binary, that's a whole different story (something something binja something). Lucky for us, simply reading the script can shed some light on almost everything we need.

### Digging In

The malicious applescript contains about 427 lines of code. In this code are several defined functions meant to determine the size of files, extract files names from paths, writing out text in the correct format, and many other custodial tasks. For example here is their take on a simple function to determine if something is a directory:
...
This fingerprinting block also contains the line to check for whether or not this script is executing from within a VM. I can tell you from personal experience this doesn't actually stop it from executing in a VM half the time. 
...
The interesting thing about the above block is that it uses a Windows user agent string while this executes on a MacOS system. This leaves us with a really good breadcrump to hunt for in our own emvironments. I am not 100% positive of the purpose of the fake ledger and trezor apps at this time. If I had to wager a guess, I would assume they are for some sort of persistence, or simply further instructions packaged as later stage executables. I have been wrong before though.
